...
;
; Version 0.2 - Simulate a high quality photo like these from the National Geographic
;               Thanks to Martin Egger <martin.egger@gmx.net> for the shadow revovery and the sharpen script
;
; Modified by Santiago Fraire using the no-batch version
;

(define (elsamuko-national-geographic-fixed-batch pattern shadowopacity
  ...
  (let* ((filelist (cadr (file-glob pattern 1))))
    (while (not (null? filelist))
      (let* ((filename (car filelist))
             ...
             (nfilename (string-append customfilename "_ng.jpg"))
        ...
        (gimp-file-save RUN-NONINTERACTIVE img adraw nfilename nfilename)
